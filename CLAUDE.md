...
- Single-page web app, **one self-contained HTML file** (`index.html`) containing inline CSS + HTML + vanilla JS. No framework, no bundler.
- PWA shell: `manifest.webmanifest`, `sw.js`, and `icons/` (192/512/180/32 PNG generated by `generate-icons.js`). Service worker is registered at end of the main `<script>` block; same-origin only, never proxies the Gemini API.
...
- The HTML is no longer version-suffixed (was `Macro_Tracker_Interactive-32.html`) — deployment expects `index.html`. The JSON DB file remains version-suffixed (`food_database-7.json`); bumping that suffix is the release mechanism for DB seed data. Edit the existing numbered file in place; only bump when explicitly asked.
...
- **Smoke test for script-region edits:** after any change to the main `<script>` block, run: `node -e "const m=require('fs').readFileSync('index.html','utf8').match(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)||[];const main=m.filter(s=>!/\ssrc=/.test(s)).pop();new Function(main.replace(/^<script[^>]*>|<\/script>$/g,''));console.log('OK')"`. Takes ~3 seconds. Catches syntax errors and orphaned content before commit. (Picks the last inline `<script>` block — the main one — skipping CDN `<script src="…">` tags, and tolerates attributes like `defer`/`nonce`. Keep the SW-registration code inside the main `<script>` block so it stays the last inline block, otherwise the smoke test will validate the wrong region.)
...
- Don't split the HTML into separate JS/CSS files — single-file delivery for the app code is the intended design. The PWA shell files (`manifest.webmanifest`, `sw.js`, `icons/`) are deployment artifacts, not architectural splits, and stay alongside `index.html`.
